...
from privacy_accountant import coreset_params
from privacy_accountant import lsh
  ...
  def children(self) -> List["LshTreeNode"]:
    ...
    return [
        LshTreeNode(self.hash_prefix + next_hash_char,
                    ...
                    self.sim_hash, self.seed) for next_hash_char,
...
def root_node(data: clustering_params.Data,
  ...
  return LshTreeNode(
      "", data.datapoints, coreset_param, sim_hash, private_count=private_count, seed=seed)
...
class LshTree():
  ...
  def __init__(self, root: LshTreeNode, seed: int):
  ...
  def print_tree(self):
    ...
    while nodes:
      print ("level", level, [n.private_count for n in nodes])
      level += 1 
      nodes = self.tree.get(level, [])
