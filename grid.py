...
X1 = pd.DataFrame(rng.multivariate_normal(mean=(0.5,0.5), cov=[[1,0],[0,1]], size=200))
X2 = pd.DataFrame(rng.multivariate_normal(mean=(-0.5,-0.5), cov=[[1,0],[0,1]], size=150))
X = pd.concat([X1, X2])
p = create_grid_synopsis(X, 1, 2, M=3)
